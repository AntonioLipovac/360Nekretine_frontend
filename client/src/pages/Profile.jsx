...
export default function Profile() {
  ...
  return (
    <div className='p-3 max-w-lg mx-auto'>
      ...
      <form onSubmit={handleSubmit} className='flex flex-col gap-4'>
        ...
        <p className='text-sm self-center'>
          {fileUploadError ? (
            ...
            <span className='text-slate-800'>{`Učitavanje ${filePerc}%`}</span>
          ) : filePerc === 100 ? (
            <span className='text-green-700'>Slika uspješno učitana!</span>
          ) : (
            ''
          )}
        </p>
      ...
      <button onClick={handleShowListings} className='text-slate-800 w-full font-semibold'>
        Prikaži oglase
      </button>
      <p className='text-red-700 mt-5'>
        {showListingsError ? 'Greška pri prikazu oglasa' : ''}
      </p>
            ...
            >
              ...
              <div className='flex flex-col items-center'>
                ...
                <Link to={`/update-listing/${listing._id}`}>
                  <button className='text-green-700 uppercase'>Uređivanje</button>
                </Link>
              </div>
            </div>
          ))}
